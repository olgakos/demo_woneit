...
## :watermelon: Локальный запуск моих тестов из Jenkins 
...
2. Слева внизу кликнуть на верхнюю желкутю иконку "Allure Report" и ознакомиться в графическим очетом о результатах последнего запуска пакета тестов.  
<p align="center">
<img title="Allure Graphics" src="images/screens/ScreenshotJ.jpg" alt="Allure Button" width="400">
</p>
...
6. Кликнуть на желкутю иконку "Allure Report" и получить свежий Allure Report. NB! срок хранения сборки на демо-сервисе ограничен. Сборка может быть деактвирована ~через 8 недель.  
...
Дополнительные параметры сборки, задействованные для выгрузки мини-отчета в Telegram:
...
Визуальный (графический) отчет с результатами прогон тествых наборов. 
<br>Способ 1: Запросить локальный отчет средствами среды разработки PyCharm(*) Для корректной работы, в большинстве случаем может потребоваться углубленная настройка локального проекта.
`
